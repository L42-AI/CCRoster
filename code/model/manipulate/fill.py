...
from helpers import get_weeknumber, id_employee, get_random_employee, get_random_shift
...
class Fill:
    ...
    def fill(Schedule: AbsSchedule):
        filled = 0
        while filled < len(Schedule):
            shift_id = get_random_shift()

            if Schedule[shift_id] is not None:
                continue

            employee_id = get_random_employee(shift_id)
